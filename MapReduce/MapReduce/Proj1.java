...
public class Proj1{
    ...
    public static class Map1 extends Mapper<WritableComparable, Text, Text, DoublePair> {
        ...
        @Override
		  ...
		  while(matcher.find()){
		      if(matcher.group().toLowerCase().compareTo(targetGram) == 0){
		          list.add(index);
		      }
		      index++;
		  }
		  ...
		  while(matcher.find()){
		      ...
		      if(absent){
			   ...
			   if((indexls < list.size())&&(index == list.get(indexls))){
			       indexls++;
				   index++;
				continue;
			   }else{
			   	if(indexls == 0){
				    d = Math.abs(index - list.get(indexls));
				}else if((indexls > 0)&&(indexls < list.size())){
				    d = Math.min(Math.abs(index - list.get(indexls)),Math.abs(index - list.get(indexls -1)));
				}else if(indexls == (list.size() -1)){
				    d = Math.abs(index - list.get(indexls));
				}else{
					d = Math.abs(index - list.get(indexls - 1));
				}
			   }
			   
			   }
		      
		      context.write( new Text(s) , new DoublePair(-1.0, func.f(d)));
		      index++;
			  }
    ...
    public static class Combine1 extends Reducer<Text, DoublePair, Text, DoublePair> {
        ...
        @Override
		   ...
		   for(DoublePair value : values) {
			sumA += value.getDouble1();
			sumS += value.getDouble2();
		   }
		   context.write(key, new DoublePair(sumA, sumS));
            }
    }
    ...
    public static class Reduce1 extends Reducer<Text, DoublePair, DoubleWritable, Text> {
        @Override
		  ...
		  //arithmetic.....
		  if((sumS > 0)&&(sumA !=0)){
		      rate = sumS*(Math.pow((Math.log(sumS)),3))/sumA;
		  }else{
		      rate = 0;
		  }
                    ...
                    Context context) throws IOException, InterruptedException {
						...
						if(n < N_TO_OUTPUT){
							for(Text value: values){
								context.write( new DoubleWritable(  Math.abs(key.get())), new Text(value));
